...
def sync_board(board_id: str, client: TrelloClient | None = None) -> SyncBoardResult:
    ...
    with transaction.atomic():
        ...
        action_count = _upsert_actions(board, actions_payload, member_map)
...
def _upsert_cards(
    ...
    for card_payload in cards_payload:
        ...
        assignees = [member_map[member_id] for member_id in assignee_ids if member_id in member_map]
        ...
        if status_changed:
            status_history_count += _record_card_status_history(card, CardStatusHistory.Source.SYNC)
